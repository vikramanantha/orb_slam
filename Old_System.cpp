    ...
    mbActivateLocalizationMode(false), mbDeactivateLocalizationMode(false), mbShutDown(false)
{
    ...
    cout << "Inputttt sensor was set to: ";
    ...
    cout << "TESTER BESTE" << endl;
    //Check settings file
    if (strSettingsFile.empty()) {
        cerr << "Error: Settings file path is empty" << endl;
        exit(-1);
    } else {
        cout << "Settings file path is not empty" << endl;
    }

    {
        cout << "Checking settings file at: " << strSettingsFile << endl;
        ifstream f(strSettingsFile.c_str());
        if (!f.good()) {
            cerr << "Error: Settings file does not exist or is not accessible at: " << strSettingsFile << endl;
            exit(-1);
        }
        f.close();
    }
    cout << "So far so good" << endl;
    cv::FileStorage* fsSettings = new cv::FileStorage();
    cout << "So far so gooood" << endl;
    try {
        cout << "Attempting to open settings file at: " << strSettingsFile << endl;
        bool opened = fsSettings->open(strSettingsFile.c_str(), cv::FileStorage::READ);
        if (!opened) {
        ...
        // Verify file is valid YAML and has content
        if (fsSettings->empty()) {
            cerr << "Settings file is empty or invalid YAML format" << endl;
            fsSettings->release();
            delete fsSettings;
            exit(-1);
        }

        cout << "Settings file opened successfully" << endl;
        
        // Test reading a simple value
        cv::FileNode version = (*fsSettings)["File.version"];
        if (version.empty()) {
            cerr << "Could not read File.version from settings" << endl;
            fsSettings->release();
            delete fsSettings;
            exit(-1);
        }
        cout << "Read File.version: " << version.string() << endl;

        // Continue with the rest of your code using fsSettings pointer
        // ...

        fsSettings->release();
        delete fsSettings;
    } catch (const cv::Exception& e) {
        cerr << "OpenCV exception while reading settings: " << e.what() << endl;
        if (fsSettings) {
            fsSettings->release();
            delete fsSettings;
        }
        ...
        cerr << "Error while opening settings file: " << e.what() << endl;
        if (fsSettings) {
            fsSettings->release();
            delete fsSettings;
        }
    ...
    cout << "Settings file open checked" << endl;
    cv::FileNode node = (*fsSettings)["File.version"];
    cout << "Checking settings file version..." << endl;
    if(!node.empty() && node.isString() && node.string() == "1.0"){
        cout << "Settings file version is 1.0" << endl;
        settings_ = new Settings(strSettingsFile,mSensor);
        cout << "Created new Settings object" << endl;

        mStrLoadAtlasFromFile = settings_->atlasLoadFile();
        cout << "Got atlas load file path" << endl;
        mStrSaveAtlasToFile = settings_->atlasSaveFile();
        cout << "Got atlas save file path" << endl;

        cout << (*settings_) << endl;
    }
    else{
        cout << "Settings file version is not 1.0 or not found" << endl;
        settings_ = nullptr;
        cv::FileNode node = (*fsSettings)["System.LoadAtlasFromFile"];
        if(!node.empty() && node.isString())
        {
            mStrLoadAtlasFromFile = (string)node;
            cout << "Got legacy atlas load file path" << endl;
        }
        ...
        if(!node.empty() && node.isString())
        {
            ...
            cout << "Got legacy atlas save file path" << endl;
        }
    }

    node = (*fsSettings)["loopClosing"];
    bool activeLC = true;
    cout << "Checking loop closing settings..." << endl;
    if(!node.empty())
    {
        activeLC = static_cast<int>((*fsSettings)["loopClosing"]) != 0;
        cout << "Loop closing is " << (activeLC ? "enabled" : "disabled") << endl;
    }

    mStrVocabularyFilePath = strVocFile;
    cout << "Set vocabulary file path" << endl;

    bool loadedAtlas = false;

    if(mStrLoadAtlasFromFile.empty())
    {
        cout << "No atlas file specified for loading" << endl;
        //Load ORB Vocabulary
        ...
        cout << "Created KeyFrame database" << endl;

        //Create the Atlas
        cout << "Initialization of Atlas from scratch " << endl;
        mpAtlas = new Atlas(0);
        cout << "Created new Atlas" << endl;
    }
    else
    {
        cout << "Atlas file specified for loading" << endl;
        //Load ORB Vocabulary
        cout << endl << "Loading ORB Vocabulary. This could take a while..." << endl;
        ...
        cout << "Attempted to load Atlas from file" << endl;
        ...
        loadedAtlas = true;
        cout << "Successfully loaded Atlas from file" << endl;

        mpAtlas->CreateNewMap();
        cout << "Created new map in Atlas" << endl;
    }


    if (mSensor==IMU_STEREO || mSensor==IMU_MONOCULAR || mSensor==IMU_RGBD) {
        mpAtlas->SetInertialSensor();
        cout << "Set inertial sensor in Atlas" << endl;
    }

    //Create Drawers. These are used by the Viewer
    mpFrameDrawer = new FrameDrawer(mpAtlas);
    cout << "Created Frame drawer" << endl;
    mpMapDrawer = new MapDrawer(mpAtlas, strSettingsFile, settings_);
    cout << "Created Map drawer" << endl;
    ...
    cout << "Created Local Mapper" << endl;
    mptLocalMapping = new thread(&ORB_SLAM3::LocalMapping::Run,mpLocalMapper);
    cout << "Launched Local Mapping thread" << endl;
    ...
    cout << "Created Loop Closer" << endl;
    mptLoopClosing = new thread(&ORB_SLAM3::LoopClosing::Run, mpLoopCloser);
    cout << "Launched Loop Closing thread" << endl;

    //Set pointers between threads
    mpTracker->SetLocalMapper(mpLocalMapper);
    mpTracker->SetLoopClosing(mpLoopCloser);
    cout << "Set Tracker connections" << endl;

    mpLocalMapper->SetTracker(mpTracker);
    mpLocalMapper->SetLoopCloser(mpLoopCloser);
    cout << "Set Local Mapper connections" << endl;

    mpLoopCloser->SetTracker(mpTracker);
    mpLoopCloser->SetLocalMapper(mpLocalMapper);
    cout << "Set Loop Closer connections" << endl;

    //Initialize the Viewer thread and launch
    if(bUseViewer)
    {
        mpViewer = new Viewer(this, mpFrameDrawer,mpMapDrawer,mpTracker,strSettingsFile,settings_);
        cout << "Created Viewer" << endl;
        mptViewer = new thread(&Viewer::Run, mpViewer);
        cout << "Launched Viewer thread" << endl;
        mpTracker->SetViewer(mpViewer);
        mpLoopCloser->mpViewer = mpViewer;
        mpViewer->both = mpFrameDrawer->both;
        cout << "Set Viewer connections" << endl;
    }

    // Fix verbosity
    Verbose::SetTh(Verbose::VERBOSITY_QUIET);
    cout << "Set verbosity level" << endl;
